...
> **102 个 Kotlin 源文件 · 3 个主页面 · MVVM + Clean Architecture**
...
graph TB
    subgraph UI["UI 层 (Jetpack Compose + Material 3)"]
        direction LR
        Home[首页<br/>扫描/检测/结果三合一]
        Recycle[回收站]
        Settings[设置<br/>可折叠分组]
        Detail[详情页]
    end
...
| 底部导航栏 | 功能 |
|-----------|------|
| 🏠 首页 | 扫描图片 → 检测重复 → 展示分组结果（三合一），支持左右滑动切换页面 |
| 🗑️ 回收站 | 查看已删除图片、恢复、永久删除 |
| ⚙️ 设置 | 可折叠分组（扫描/去重/删除/升级/通用/关于），跟随系统语言 |

- **页面切换**：底部导航栏切换 + 左右滑动均支持，带滑动过渡动画
- **首次启动流程**：请求存储权限 → 同意隐私政策/服务条款 → 进入首页
- **用户信息安全**：扫描完成不自动去重，需用户手动点击"开始检测重复图片"

### 模块依赖关系

```mermaid
graph TD
    App["app<br/>(主应用模块<br/>首页集成扫描/检测/结果)"]
...
| 设置（DataStore） | ✅ 100% | 可折叠分组，深色/浅色/跟随系统主题 |
| 首页（扫描/检测/结果三合一） | ✅ 100% | HorizontalPager 左右滑动，手动触发去重 |
| 系统适配 | ✅ 100% | 权限分版本/分区存储/前台 Service/FileProvider |
| 性能优化 | ✅ 100% | 设备分级/LSH O(n)/流式 Flow/降级策略 |
| 异常处理 | ✅ 100% | 分级日志/文件轮转/WorkManager 重试/全局崩溃捕获 |
| 隐私合规 | ✅ 100% | 首次启动权限请求 + 隐私政策/服务条款同意 |
| 主链路 | ✅ 100% | 首页→扫描→检测→结果→回收站→设置 完整闭环 |
